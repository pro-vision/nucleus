  ...
  function copyMarkupFromIframes() {
    ...
    $('iframe[id^="ifrm_"]:not([' + loadedFlag + '])').each(function(n, iframe) {
      ...
      if (iframeDoc.readyState === "loading") {
        ...
        var markupSelector = iframe.getAttribute('data-markup-selector');
        var html = '';

        if (markupSelector) {
          try {
            html = [].reduce.call(
              iframeBody.querySelectorAll(markupSelector),
              function (accumulator, currentNode) {
                return accumulator + '\n' + currentNode.outerHTML.trim();
              },
              "");
          }
          catch (e) {}
        ...
        if (!html) {
          html = iframeBody.innerHTML.trim();
        }
        ...
        var syntaxHighlightedHTML = Prism.highlight(html, Prism.languages.html);
